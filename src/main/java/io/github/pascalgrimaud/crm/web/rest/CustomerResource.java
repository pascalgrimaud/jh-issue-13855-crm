...
import io.github.pascalgrimaud.crm.service.CustomerService;
...
/**
 * REST controller for managing {@link io.github.pascalgrimaud.crm.domain.Customer}.
 */
@RestController
@RequestMapping("/api")
public class CustomerResource {
    ...
    /**
     * {@code PATCH  /customers} : Updates given fields of an existing customer.
     *
     * @param customer the customer to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated customer,
     * or with status {@code 400 (Bad Request)} if the customer is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/customers", consumes = "application/merge-patch+json")
    public ResponseEntity<Customer> partialUpdateCustomer(@NotNull @RequestBody Customer customer) throws URISyntaxException {
        log.debug("REST request to update Customer partially : {}", customer);
        if (customer.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
