...
import io.github.pascalgrimaud.crm.service.ShipmentService;
import io.github.pascalgrimaud.crm.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link io.github.pascalgrimaud.crm.domain.Shipment}.
 */
@RestController
@RequestMapping("/api")
public class ShipmentResource {
    ...
    private static final String ENTITY_NAME = "crmShipment";
    ...
    /**
     * {@code PATCH  /shipments} : Updates given fields of an existing shipment.
     *
     * @param shipment the shipment to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated shipment,
     * or with status {@code 400 (Bad Request)} if the shipment is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/shipments", consumes = "application/merge-patch+json")
    public ResponseEntity<Shipment> partialUpdateShipment(@NotNull @RequestBody Shipment shipment) throws URISyntaxException {
        log.debug("REST request to update Shipment partially : {}", shipment);
        if (shipment.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
