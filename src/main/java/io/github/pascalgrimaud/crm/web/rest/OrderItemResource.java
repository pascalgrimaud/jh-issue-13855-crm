...
import io.github.pascalgrimaud.crm.service.OrderItemService;
...
/**
 * REST controller for managing {@link io.github.pascalgrimaud.crm.domain.OrderItem}.
 */
@RestController
@RequestMapping("/api")
public class OrderItemResource {
    ...
    /**
     * {@code PATCH  /order-items} : Updates given fields of an existing orderItem.
     *
     * @param orderItem the orderItem to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated orderItem,
     * or with status {@code 400 (Bad Request)} if the orderItem is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/order-items", consumes = "application/merge-patch+json")
    public ResponseEntity<OrderItem> partialUpdateOrderItem(@NotNull @RequestBody OrderItem orderItem) throws URISyntaxException {
        log.debug("REST request to update OrderItem partially : {}", orderItem);
        if (orderItem.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
