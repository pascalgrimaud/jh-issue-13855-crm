...
import io.github.pascalgrimaud.crm.service.InvoiceService;
...
/**
 * REST controller for managing {@link io.github.pascalgrimaud.crm.domain.Invoice}.
 */
@RestController
@RequestMapping("/api")
public class InvoiceResource {
    ...
    private static final String ENTITY_NAME = "crmInvoice";
    ...
    /**
     * {@code PATCH  /invoices} : Updates given fields of an existing invoice.
     *
     * @param invoice the invoice to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated invoice,
     * or with status {@code 400 (Bad Request)} if the invoice is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/invoices", consumes = "application/merge-patch+json")
    public ResponseEntity<Invoice> partialUpdateInvoice(@NotNull @RequestBody Invoice invoice) throws URISyntaxException {
        log.debug("REST request to update Invoice partially : {}", invoice);
        if (invoice.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
