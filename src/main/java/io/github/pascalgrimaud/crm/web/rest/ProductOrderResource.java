...
import io.github.pascalgrimaud.crm.service.ProductOrderService;
...
/**
 * REST controller for managing {@link io.github.pascalgrimaud.crm.domain.ProductOrder}.
 */
@RestController
@RequestMapping("/api")
public class ProductOrderResource {
    ...
    /**
     * {@code PATCH  /product-orders} : Updates given fields of an existing productOrder.
     *
     * @param productOrder the productOrder to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated productOrder,
     * or with status {@code 400 (Bad Request)} if the productOrder is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/product-orders", consumes = "application/merge-patch+json")
    public ResponseEntity<ProductOrder> partialUpdateProductOrder(@NotNull @RequestBody ProductOrder productOrder)
        throws URISyntaxException {
        log.debug("REST request to update ProductOrder partially : {}", productOrder);
        if (productOrder.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
